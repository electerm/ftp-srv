/**
 * Integration test for dist/cjs build using Node.js native test runner.
 * Uses basic-ftp (promise-based) for real FTP operations.
 * Run: node --test test-cjs.js
 */
...
const FtpServer = require('../dist/cjs/index.js').default;
...
describe('dist/cjs', () => {
  let server;
  const dir = path.join(os.tmpdir(), 'ftp-srv-int-cjs');
  ...
  test('STOR uploads a file and RETR downloads it', async () => {
    const content = 'hello from cjs integration test';
    await withClient(async (client) => {
